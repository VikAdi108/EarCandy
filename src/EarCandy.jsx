...
export default function EarCandy() {
  ...
  const generateRecommendations = useCallback(() => {
    if (!selectedMood || !detectedFeatures) return;
    
    const mood = moods.find(m => m.id === selectedMood);
    
    // Score each track based on mood match and acoustic similarity
    const scored = sampleTracks.map(track => {
      let score = 0;
      
      // Mood match (primary factor for psychology layer)
      if (track.mood === selectedMood) score += 50;
      
      // Tempo similarity (physics layer)
      const tempoDiff = Math.abs(track.bpm - detectedFeatures.estimatedTempo);
      score += Math.max(0, 30 - tempoDiff / 3);
      
      // Add some randomness for variety
      score += Math.random() * 20;
      
      // Genre diversity bonus - prefer spreading across genres
      return { ...track, score };
    });
